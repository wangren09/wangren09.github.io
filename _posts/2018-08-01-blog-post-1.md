...
$p(x) = \prod_{i=1}^n p(x_{i}|x_1,...,x_{i-1})$
...
In addition to decoder network modeling $p_{\theta}(x|z)$, define addidional encoder netowrk $q_{\Phi}$ that approximates $p_{\theta}(z|x)$.

Encoder Network $q_{\theta}(z|x)$ (parameters $\Phi$): $x$ --> $\mu_{z|x}$, $\Sigma_{z|x}$ (mean and (diagonal) covariance of $z|x$)

* Sample $z$ from $z|x \sim N(\mu_{z|x}, \Sigma_{z|x})$ 

Decoder Network $p_{\theta}(x|z)$ (parameters $\theta$): $z$ --> $\mu_{x|z}$, $\Sigma_{x|z}$ (mean and (diagonal) covariance of $x|z$)
...
* $E_z[\log p_{\theta}(x^{(i)}|z)]$: Decoder network gives $p_{\theta}(x|z)$, can compute estimate of this term through sampling

* $D_{KL}(q_{\Phi}(z|x^{(i)})||p_{\theta}(z))$: This KL term (between Gaussians for encoder and $z$ prior) has nice closed-form solution

* $D_{KL}(q_{\Phi}(z|x^{(i)})||p_{\theta}(z|x^{(i)}))$: $p_{\theta}(z|x)$ intractable, cannot compute this KL term, but we know KL divergence always $\ge0$ 
