...
### Generative Adversarial Networks (GANs)
 ...
 $\min_{\theta_{g}} \max_{\theta_{d}}[E_{x \thicksim p_{data}} \log D_{\theta_{d}}(x) + E_{z \thicksim p(z)} \log(1-D_{\theta_{d}}(G_{\theta_{g}}(z)))]$
 
 * Discriminator ($\theta_{d}$) wants to maximize objective such that $D(x)$ is close to 1 (real) and $D(G(z))$ is close to 0 (fake)

 * Generator ($\theta_{g}$) wants to minimize objective such that $D(G(z))$ is close to 1 (discriminator is fooled into thinking generatored $G(z)$ is real)

Alternate between:

* Gradient ascent on discriminator

  $\max_{\theta_{d}}[E_{x \thicksim p_{data}} \log D_{\theta_{d}}(x) + E_{z \thicksim p(z)} \log(1-D_{\theta_{d}}(G_{\theta_{g}}(z)))]$ 
  
* Gradient descent on generator

  $\min_{\theta_{g}} E_{z \thicksim p(z)} \log(1-D_{\theta_{d}}(G_{\theta_{g}}(z)))$
  
or
  
*  Gradient ascent on generator
  
  $\max_{\theta_{g}} E_{z \thicksim p(z)} \log(D_{\theta_{d}}(G_{\theta_{g}}(z)))$
...
The mapping from inputs to outputs: close to linear (nearly linear responses in  practice)

The mapping from parameters to outputs: non-linear

The Fast Gradient Sign Method

$J(\bar{x}, \theta) \approx J(x, \theta) + (\bar{x}-x)^T \nabla_x J(x)$

Maximize $J(x, \theta) + (\bar{x}-x)^T \nabla_x J(x)$

subject to $\|\bar{x} - \|_{\inf} \le \epsilon$

We then have $\bar{x} = x + \epsilon sign(\nabla_x J(x))$
